...
function Home() {
    ...
    useEffect(()=>{
        ...
        if(localStorage.getItem("token")){
            Axios.get(`http://54.86.121.253:9000/testUser/${localStorage.getItem("token")}`).then((response)=>{
    ...
    const updateUser=()=>{
        Axios.put(`http://54.86.121.253:9000/updateUser/${localStorage.getItem("token")}`,{
            name,
            bio,
            username,
            password,
